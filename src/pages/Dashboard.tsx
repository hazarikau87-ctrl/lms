...
import {
  ...
  MessageCircle, Building2, FileDown, CheckCircle2, RotateCw, Edit3, X, XCircle, Settings as SettingsIcon, LayoutDashboard, MapPin, Beaker, Save, User, Sun, Sunrise, Sunset
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // Side-Drawer Slide-over workflow states
  ...
  const handleOpenEditDrawer = (item: Appointment & { whatsapp?: string }) => {
    setEditingAppointment(item);
    setEditFormDate(item.appointment_date || '');
    setEditFormTime(item.time || '08:00');
    setEditFormWhatsApp(item.whatsapp || item.mobile || '');
  };
  ...
  const handleCommitBookingEdits = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Inline UI patch mapping updates dynamically
  ...
  const deleteBooking = async (id: number) => {
    ...
    await supabase.from('appointments').update({ is_deleted: true, deleted_at: new Date().toISOString() }).eq('id', id).eq('lab_id', lab.id);
    fetchAll();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-blue-500 selection:text-white antialiased">
        ...
        {currentView === 'settings' ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <StatCard icon={<CalendarCheck className="w-4 h-4 text-blue-600" />} iconBg="bg-blue-50" value={stats.total} label={selectedDate ? `Booked on ${selectedDate}` : "Total Bookings"} isActive={statusFilter === 'all'} onClick={() => handleStatusFilterClick('all')} />
              <StatCard icon={<Clock className="w-4 h-4 text-amber-600" />} iconBg="bg-amber-50" value={stats.pending} label="Pending Requests" isActive={statusFilter === 'pending'} onClick={() => handleStatusFilterClick('pending')} />
              <StatCard icon={<CheckCheck className="w-4 h-4 text-emerald-600" />} iconBg="bg-emerald-50" value={stats.completed} label="Completed Tests" isActive={statusFilter === 'completed'} onClick={() => handleStatusFilterClick('completed')} />
              <StatCard icon={<XCircle className="w-4 h-4 text-rose-600" />} iconBg="bg-rose-50" value={stats.cancelled} label="Cancelled" isActive={statusFilter === 'cancelled'} onClick={() => handleStatusFilterClick('cancelled')} />
            </div>
      ...
      {/* PREMIUM RIGHT SLIDE-OVER EDIT DRAWERS SHEET PANEL */}
      {editingAppointment && (
        <div 
...
function AppointmentRow({ item, selected, onToggle, onUpdateStatus, onUpdateRemarks, onDelete, onWhatsApp, onViewAddress, onViewTests, onEditBooking }: AppointmentRowProps) {
    ...
    <tr className={`hover:bg-slate-50/50 transition-colors ${selected ? 'bg-blue-50/20' : ''}`}>
      ...
      <td className="px-4 py-3.5 whitespace-nowrap">
        <button
          ...
          className="font-mono text-xs font-bold text-blue-600 hover:text-blue-800 hover:underline tracking-tight focus:outline-none text-left"
      ...
      <td className="px-4 py-3.5">
        <div className="relative group max-w-[160px]">
          <textarea value={localRemarks} onChange={(e) => setLocalRemarks(e.target.value)} onBlur={handleRemarksBlur} placeholder="Add log entry..." rows={1} className="w-full text-[11px] font-medium p-1 bg-transparent border-b border-transparent hover:border-slate-200 focus:bg-slate-50 focus:border-slate-200 focus:p-1.5 rounded outline-none resize-none transition-all custom-scrollbar" />
